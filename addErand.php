<?php
	...
	try{
        ...
        $password = $data->erandpassword;
        	...
        	$dataa = $connection->prepare('select * from erand where username=:username and password=:password');	
		    ...
		    $connection = new PDO(DB_DRIVER . ":dbname=" . DB_DATABASE . ";host=" . DB_SERVER, DB_USER, DB_PASSWORD);
	  	    $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$dataa =$connection-> prepare('insert into erand(firstname,lastname,phone,email,username,password)values(:firstname,:lastname,:phone,:email,:username,:password)');
			...
			if ($res) {
				$arr = array('msg' => "Added New Erand Transporter  ".$firstname, 'error' => '');
                $jsn = json_encode($arr);
                print_r($jsn);
			}else{
