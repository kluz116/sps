<?php
	...
	try{
        ...
        $password = $data->partnerpassword;
        	...
        	$dataa = $connection->prepare('select * from partners where username=:username and password=:password');	
		    ...
		    $connection = new PDO(DB_DRIVER . ":dbname=" . DB_DATABASE . ";host=" . DB_SERVER, DB_USER, DB_PASSWORD);
	  	    $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$dataa =$connection-> prepare('insert into partners(partnername,partnerphonee,email,username,password)values(:firstname,:phone,:email,:username,:password)');
			...
			if ($res) {
				$arr = array('msg' => "Added New Partner  ".$firstname, 'error' => '');
                $jsn = json_encode($arr);
                print_r($jsn);
			}else{
